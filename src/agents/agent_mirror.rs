...
lazy_static! {
    static ref HISTORY: Mutex<Vec<Observation>> = Mutex::new(Vec::new());
}
...
pub fn agent_mirror(obs: Observation, conf: Configuration) -> u8 {
    let last_obs = HISTORY.lock().unwrap().last().cloned();
    let mut action;
    {
        let board = BoardArray::from(obs.board);
        action = match get_opponent_action(&obs, last_obs) {
    ...
    HISTORY.lock().unwrap().push(obs);  // Save history
    action
}
...
mod test {
    ...
    #[case(5, 1)]
    #[case(6, 0)]
    fn test_move_mirror(action_p1: GameCol, action_p2: GameCol) {
        clear_history();
