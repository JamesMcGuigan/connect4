...
pub fn agent_minimax(obs: Observation, _conf: Configuration) -> u8 {
    ...
    for col in board.get_valid_actions() {
        if let Some(state) = board.step(col) {
            let score = minimax(&*state, depth, i32::MIN + 1, i32::MAX - 1, board.get_move_player());
            if score > max_score {
                max_score = score;
                max_col = col;
            }
...
fn minimax(state: &dyn Board, depth: u8, mut alpha: i32, mut beta: i32, root_player: PlayerID) -> i32 {
    ...
    if depth == 0 || actions.is_empty() || state.terminated() {
        let opponent_id = get_opponent_id(root_player).unwrap();
        if state.is_win(root_player) { return i32::MAX; }
        if state.is_win(opponent_id) { return i32::MIN; }
        return state.huristic_score(root_player) as i32;
    }
    ...
    if is_maximizing_player {
        ...
        for mv in actions {
            if let Some(next) = state.step(mv) {
                let eval = minimax(&*next, depth - 1, alpha, beta, root_player);
